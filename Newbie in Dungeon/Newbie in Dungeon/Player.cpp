...
Player::Player(std::string initialName, int initialHp, int initialAttack, int initialDefense):
	...
	exp(0),
	requiredExp(100),	
	bandageCount(0),     // GameManager에서 지급할 것이므로 0으로 시작// --- 6번: 치료 붕대 초기화 ---
	bandageHealAmt(20),  // 치료 붕대 사용 시 회복량 (예시: 20)
	potionCount(0),      // 레벨업 시 충전될 것이므로 0으로 시작// --- 3번: 포션 초기화 ---
	potionHealAmt(50),    // 포션 사용 시 회복량 (예시: 50)
	isDefending(false), // 방어 상태 초기화	// --- 새로운 전투 행동 관련 멤버 변수 초기화 ---
	strongAttackCooldown(0), // 강공격 쿨다운 초기화
	selectedSpecialAbility(nullptr)
	{
...
void Player::ResetTurnState() {
	...
	if (selectedSpecialAbility) { // 선택된 특수능력이 있다면 쿨다운 감소
		selectedSpecialAbility->decreaseCooldown();
	}
}
// 특수능력 설정 함수
void Player::setSpecialAbility(std::unique_ptr<SpecialAbility> ability) {
	selectedSpecialAbility = std::move(ability);
	if (selectedSpecialAbility) {
		std::cout << GetName() << " (이)가 특수능력 [" << selectedSpecialAbility->getName() << "]을(를) 선택했습니다." << std::endl; // 가독성 개선
	}
	else {
		std::cout << GetName() << " (이)가 특수능력을 선택하지 않았습니다." << std::endl; // 가독성 개선
	}
}

// 설정된 특수능력 사용 함수
void Player::useSpecialAbility(Monster& targetEnemy) {
	if (!selectedSpecialAbility) {
		std::cout << "선택된 특수능력이 없습니다." << std::endl;
		return;
	}
	selectedSpecialAbility->use(*this, targetEnemy); // 선택된 특수능력 사용
}
...
{
	{
		std::cout << "--- " << GetName() << "의 상태 ---" << std::endl;
